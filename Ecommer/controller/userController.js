...
const loginUserCtrl = asyncHandler (async(req, res) => {
    ...
    if(findUser && await findUser.isPasswordMatched(password)){
        const refresToken = await generateRefreshToken(findUser?._id);
...
const loginAdmin = asyncHandler (async(req, res) => {
    ...
    if(findAdmin.role !== 'admin') throw new Error("Not Autherized");
    if(findAdmin && await findAdmin.isPasswordMatched(password)){
        const refresToken = await generateRefreshToken(findAdmin?._id);
...
const handleRefreshToken = asyncHandler(async (req, res) => {
    ...
    if(!cookie?.refresToken) throw new Error('No refresh Token in cookie');
...
const forgotPasswordToken = asyncHandler(async (req, res) =>{
    ...
    try{
        ...
        const resetURL = `Please follow this link to reset your password
                        ...
                        <a href='https://localhost:3000/api/user/resetpassword${token}'>Click here</a>`;
